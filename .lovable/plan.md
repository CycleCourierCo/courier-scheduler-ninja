...
- The calendar weeks read **Monday to Sunday** instead of starting on Sunday, so the working week lines up.
...
- Instead the email goes out as soon as the NI customer confirms their collection day, and it now states that date clearly ("Please collect on Tuesday 23 September") along with any collection notes the customer left.
...
- `src/components/availability/AvailabilityForm.tsx`: add `requiredDates` (default 7) and `maxDates` props; drive the validation text, selected-dates copy and submit-button guard from them so a value of 1 gives single-day selection; pass `weekStartsOn={1}` to the `Calendar` so weeks start on Monday.
...
- `supabase/functions/_shared/ferryPartnerEmail.ts`: add optional `pickup_date` and `sender_notes` to the input and, for inbound, render a "Requested collection date" block plus the customer's collection notes (escaped); `send-ferry-partner-notification/index.ts` adds both columns to its `select` and passes them through.
