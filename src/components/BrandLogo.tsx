...
export default function BrandLogo({ className, compact = false, showMobileWordmark = false }: BrandLogoProps) {
  if (compact) {
    return (
      <span className={cn("flex min-w-0 items-center gap-2", className)}>
        <img src={solidLogo.url} alt="" className="h-8 w-8 shrink-0" />
  ...
  return (
    <span className={cn("block", className)}>
      <img src={horizontalLogo.url} alt="Cycle Courier Co." className="h-full w-full object-contain object-left dark:hidden" />
      <img src={reversedHorizontalLogo.url} alt="Cycle Courier Co." className="hidden h-full w-full object-contain object-left dark:block" />
    </span>
  );
}