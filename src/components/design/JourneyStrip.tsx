...
import { CalendarCheck, MapPinCheck, PackageCheck, Truck } from "lucide-react";
...
export interface JourneyStop {
  ...
  icon?: "booked" | "collected" | "transit" | "delivered";
}
  ...
  return (
    <ol aria-label="Order journey" className={cn("grid min-w-0 gap-0", compact ? "grid-flow-col auto-cols-fr" : "grid-cols-1 sm:grid-flow-col sm:auto-cols-fr", className)}>
      {stops.map((stop, index) => {
        ...
        const MilestoneIcon = stop.icon === "booked"
          ? CalendarCheck
          : stop.icon === "collected"
            ? PackageCheck
            : stop.icon === "delivered"
              ...
              : stop.icon === "transit"
                ...
                : null;
        return (
          <li key={`${stop.label}-${index}`} className={cn("relative flex min-w-0 gap-3 pb-5 sm:block sm:pb-0", compact && "pb-0", !compact && "min-h-16")}>
