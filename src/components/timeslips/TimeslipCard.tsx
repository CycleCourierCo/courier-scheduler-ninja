...
import { Clock, MapPin, TrendingUp, Check, X, Edit, ExternalLink, Zap, Trash2 } from 'lucide-react';
    ...
    <Card className="hover:shadow-md transition-shadow min-w-0 overflow-hidden">
          ...
          <div className="flex items-start justify-between gap-3 flex-wrap">
            <div className="min-w-0 flex-1">
              <CardTitle className="flex items-center gap-2 flex-wrap">
                <span className="break-words">{timeslip.driver?.name || 'Unknown Driver'}</span>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="text-sm space-y-1 p-3 bg-muted rounded-lg">
            ...
            {isAdmin && timeslip.vehicle && (
              <div className="flex justify-between text-muted-foreground">
                <span>Vehicle:</span>
                <span className="font-medium">
                  {timeslip.vehicle.registration}
                  {timeslip.vehicle.make ? ` — ${timeslip.vehicle.make}` : ''}
                </span>
              </div>
            )}
          </div>
