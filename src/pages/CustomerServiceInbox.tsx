    ...
    <Layout>
      <div className="container mx-auto px-2 md:px-4 py-4 flex-1 flex flex-col">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h1 className="text-h4 font-semibold">Customer Service Inbox</h1>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-[320px_1fr_300px] gap-3 flex-1 min-h-[70vh]">
              ...
              <div className="flex gap-1">
                <Tabs value={scope} onValueChange={(v) => setScope(v as any)} className="flex-1">
                  <TabsList className="h-7 w-full grid grid-cols-3">
                    <TabsTrigger value="all" className="text-caption h-6">All</TabsTrigger>
                    <TabsTrigger value="mine" className="text-caption h-6">Mine</TabsTrigger>
                    <TabsTrigger value="unassigned" className="text-caption h-6">Unass.</TabsTrigger>
                  </TabsList>
                </Tabs>
              </div>
              <div className="flex gap-1">
                <Button variant={channel === 'all' ? 'default' : 'outline'} size="sm" className="h-7 px-2 text-caption flex-1" onClick={() => setChannel('all')}>All</Button>
                <Button variant={channel === 'email' ? 'default' : 'outline'} size="sm" className="h-7 px-2 text-caption flex-1" onClick={() => setChannel('email')}>
                  <Mail className="h-3 w-3 mr-1" />Email
                </Button>
                <Button variant={channel === 'whatsapp' ? 'default' : 'outline'} size="sm" className="h-7 px-2 text-caption flex-1" onClick={() => setChannel('whatsapp')}>
          ...
          <div className="border rounded-md flex flex-col bg-card overflow-hidden min-h-[400px]">
            {conversation ? (
              ...
              <div className="flex-1 flex items-center justify-center text-small text-muted-foreground p-8">
