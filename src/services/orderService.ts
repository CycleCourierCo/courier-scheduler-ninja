  ...
  try {
    ...
    if (search && search.trim()) {
      ...
      query = query.or(
        ...
        `sender->>email.ilike.%${searchTerm}%,` +
        `receiver->>email.ilike.%${searchTerm}%,` +
        `sender->>phone.ilike.%${searchTerm}%,` +
        `receiver->>phone.ilike.%${searchTerm}%,` +
        `sender->address->>zipCode.ilike.%${searchTerm}%,` +
        `receiver->address->>zipCode.ilike.%${searchTerm}%,` +
...
export const createOrder = async (data: CreateOrderFormData): Promise<Order> => {
  try {
    const { sender, receiver, bikeBrand, bikeModel, bikeType, bikeQuantity, bikes, customerOrderNumber, needsPaymentOnCollection, paymentCollectionPhone, isBikeSwap, partExchangeBikeBrand, partExchangeBikeModel, partExchangeBikeType, partExchangeBikeValue, isEbayOrder, collectionCode, deliveryInstructions, needsInspection, isBoxMyBike, boxBuyer, isWarehouseStorage } = data as any;
    ...
    const { data: order, error } = await supabase
      ...
      .insert({
        ...
        is_warehouse_storage: isWarehouseStorage || false,
...
export const setGuaranteedDelivery = async (
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      guaranteed_delivery_marked_by_name: markedBy?.name || null,
      updated_at: new Date().toISOString(),
    } as any)
    .eq("id", orderId);
...
export const clearGuaranteedDelivery = async (orderId: string): Promise<void> => {
  const { error } = await supabase
    ...
    .update({
      ...
      guaranteed_delivery_marked_by_name: null,
      updated_at: new Date().toISOString(),
    } as any)
    .eq("id", orderId);
