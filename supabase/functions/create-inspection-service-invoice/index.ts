...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    console.log('Creating retrospective inspection & service invoice for order:', orderId);
    ...
    // Find "Bike Inspection & Service" product
    const escapedProductName = escapeQuickBooksString('Bike Inspection & Service');
    ...
    let inspectionProduct: { id: string; name: string; price: number } | null = null;
    if (productResponse.ok) {
      const item = (await productResponse.json()).QueryResponse?.Item?.[0];
      if (item) {
        inspectionProduct = { id: item.Id, name: item.Name, price: item.UnitPrice || 0 };
      }
    }
    if (!inspectionProduct) {
      throw new Error('QuickBooks product "Bike Inspection & Service" not found.');
    }
    ...
    // Build description matching bulk invoice format
    const senderName = (order.sender as any)?.name || '';
    const receiverName = (order.receiver as any)?.name || '';
    ...
    const lineItems = [{
      ...
      SalesItemLineDetail: {
        ItemRef: { value: inspectionProduct.id, name: inspectionProduct.name },
        Qty: 1,
    ...
    console.error('Error creating inspection service invoice:', error);
    return new Response(
      JSON.stringify({ error: error.message || 'Failed to create inspection service invoice' }),
