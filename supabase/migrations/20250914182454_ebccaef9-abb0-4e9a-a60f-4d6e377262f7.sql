-- Fix digest function with explicit type casting
...
DECLARE
  ...
  input_key_hash text;
BEGIN
  -- Hash the provided key with explicit type casting
  input_key_hash := encode(digest(api_key::text, 'sha256'::text), 'hex');
...
-- Also fix the admin_generate_api_key function
...
BEGIN
  ...
  -- Hash the key for storage with explicit type casting
  key_hash := encode(digest(full_key::text, 'sha256'::text), 'hex');
