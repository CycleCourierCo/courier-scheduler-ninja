...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'has_foam_photos', (
      v_order.foam_delivery_photos IS NOT NULL
      AND array_length(v_order.foam_delivery_photos, 1) > 0
    ),
    'foam_delivery_photos', CASE
      WHEN p_reveal_side = 'receiver' THEN to_jsonb(v_order.foam_delivery_photos)
      ELSE NULL
    END,
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'storage' AND tablename = 'objects'
      AND policyname = 'Foam delivery photos readable for tracking'
  ) THEN
    CREATE POLICY "Foam delivery photos readable for tracking"
      ON storage.objects FOR SELECT
      TO anon, authenticated
      USING (bucket_id = 'foam-delivery-photos');
  END IF;
END $$;