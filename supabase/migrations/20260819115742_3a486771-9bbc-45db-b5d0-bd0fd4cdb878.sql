CREATE OR REPLACE FUNCTION public.set_order_availability(p_order_id uuid, p_side text, p_dates jsonb, p_notes text, p_postcode text DEFAULT NULL::text, p_alt_location jsonb DEFAULT NULL::jsonb, p_lat numeric DEFAULT NULL::numeric, p_lon numeric DEFAULT NULL::numeric)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Backfill coordinates from the postcode geocode when the stored address has none
  IF p_lat IS NOT NULL AND p_lon IS NOT NULL THEN
    v_side_json := CASE WHEN p_side = 'sender' THEN v_order.sender ELSE v_order.receiver END;
    IF v_side_json IS NOT NULL THEN
      v_existing_lat := COALESCE(v_side_json->'address'->>'lat', v_side_json->>'lat');
      v_existing_lon := COALESCE(v_side_json->'address'->>'lon', v_side_json->>'lon');
      IF v_existing_lat IS NULL OR v_existing_lon IS NULL
         OR v_existing_lat IN ('', '0') OR v_existing_lon IN ('', '0') THEN
        v_side_json := jsonb_set(
          v_side_json,
          '{address}',
          COALESCE(v_side_json->'address', '{}'::jsonb)
            || jsonb_build_object('lat', p_lat, 'lon', p_lon),
          true
        );
        IF p_side = 'sender' THEN
          UPDATE public.orders SET sender = v_side_json WHERE id = p_order_id;
          v_order.sender := v_side_json;
        ELSE
          UPDATE public.orders SET receiver = v_side_json WHERE id = p_order_id;
          v_order.receiver := v_side_json;
        END IF;
      END IF;
    END IF;
  END IF;
