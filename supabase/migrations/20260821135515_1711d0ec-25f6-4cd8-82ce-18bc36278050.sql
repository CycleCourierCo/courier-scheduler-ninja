...
DROP POLICY IF EXISTS "Staff can read inspection reports" ON storage.objects;
CREATE POLICY "Staff can read inspection reports"
ON storage.objects FOR SELECT TO authenticated
USING (bucket_id = 'inspection-reports' AND public.is_internal_staff(auth.uid()));

DROP POLICY IF EXISTS "Staff can upload inspection reports" ON storage.objects;
CREATE POLICY "Staff can upload inspection reports"
ON storage.objects FOR INSERT TO authenticated
WITH CHECK (bucket_id = 'inspection-reports' AND public.is_internal_staff(auth.uid()));

DROP POLICY IF EXISTS "Staff can update inspection reports" ON storage.objects;
...
BEGIN
  ...
  SELECT id, status, inspected_at, released_to_customer_at, report_url
  INTO v_inspection
  ...
  RETURN jsonb_build_object(
    ...
    'report_url', CASE WHEN v_public_inspected_at IS NOT NULL THEN v_inspection.report_url ELSE NULL END
  );
END;
$function$;