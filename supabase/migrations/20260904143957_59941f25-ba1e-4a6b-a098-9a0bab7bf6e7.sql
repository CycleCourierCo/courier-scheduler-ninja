...
DECLARE
  ...
  v_sender_business boolean := false;
  v_receiver_business boolean := false;
  v_sender_hours jsonb;
  v_receiver_hours jsonb;
BEGIN
  ...
  v_sender_email   := lower(trim(coalesce(v_order.sender->>'email', '')));
  v_receiver_email := lower(trim(coalesce(v_order.receiver->>'email', '')));

  IF v_sender_email <> '' THEN
    SELECT true, p.opening_hours
      INTO v_sender_business, v_sender_hours
      FROM public.profiles p
     WHERE p.is_business IS TRUE
       AND coalesce(p.account_status::text, 'approved') = 'approved'
       AND (lower(trim(coalesce(p.email, ''))) = v_sender_email
            OR lower(trim(coalesce(p.accounts_email, ''))) = v_sender_email)
     LIMIT 1;
  END IF;

  IF v_receiver_email <> '' THEN
    SELECT true, p.opening_hours
      INTO v_receiver_business, v_receiver_hours
      FROM public.profiles p
     WHERE p.is_business IS TRUE
       AND coalesce(p.account_status::text, 'approved') = 'approved'
       AND (lower(trim(coalesce(p.email, ''))) = v_receiver_email
            OR lower(trim(coalesce(p.accounts_email, ''))) = v_receiver_email)
     LIMIT 1;
  END IF;

  v_sender_business   := coalesce(v_sender_business, false);
  v_receiver_business := coalesce(v_receiver_business, false);
  ...
  || jsonb_build_object(
    ...
    'foam_crossed_to_ni_at',      v_order.foam_crossed_to_ni_at,
    'foam_delivered_ni_at',       v_order.foam_delivered_ni_at,
    'ni_direction',                   v_order.ni_direction,
    'ni_inbound_status',              v_order.ni_inbound_status,
    'ni_inbound_collected_at',        v_order.ni_inbound_collected_at,
    'ni_inbound_ferry_crossed_at',    v_order.ni_inbound_ferry_crossed_at,
    'ni_inbound_received_at',         v_order.ni_inbound_received_at,
    'ni_bfs_number',                  v_order.ni_bfs_number,
    ...
    'sender_is_business',         v_sender_business,
    'receiver_is_business',       v_receiver_business,
    'sender_opening_hours',       v_sender_hours,
    'receiver_opening_hours',     v_receiver_hours,
