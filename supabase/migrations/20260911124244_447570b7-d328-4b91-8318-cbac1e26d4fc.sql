...
BEGIN
  ...
  FOR r IN
    SELECT ins.id,
           ...
           COUNT(*) FILTER (
             WHERE i.status IN ('approved', 'resolved', 'repaired')
           ) AS any_approved,
           COUNT(*) FILTER (
  ...
  LOOP
    ...
    ELSIF r.any_approved = 0 AND r.any_declined > 0 THEN
      -- Nothing was actually repaired: the bike ships unrepaired.
      v_next := 'ship_as_is';
    ELSE
      v_next := 'repaired';
    END IF;
...
BEGIN
  ...
  v_public_inspected_at := CASE
    WHEN v_inspection.status IN ('inspected', 'repaired', 'ship_as_is')
  ...
  IF v_inspection.status = 'ship_as_is' THEN
    v_repairs_declined_at := coalesce(v_repairs_declined_at, v_inspection.updated_at);
  END IF;
